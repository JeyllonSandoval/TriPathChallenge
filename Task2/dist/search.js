...
let todasLasRazas = [];
let dogTable = null;
let currentBreedSort = null;
let currentSubBreedsSort = null;
/**
...
function ordenarRazas(razas) {
    ...
    if (currentSubBreedsSort === 'has-subbreeds-first') {
        razasOrdenadas.sort((a, b) => {
            ...
            if (aHasSubBreeds && !bHasSubBreeds)
                return -1;
            if (!aHasSubBreeds && bHasSubBreeds)
                return 1;
            return 0;
 ...
 */
function actualizarTabla(terminoBusqueda) {
 ...
 */
function manejarOrdenamientoRaza() {
 ...
 */
function manejarOrdenamientoSubRazas() {
